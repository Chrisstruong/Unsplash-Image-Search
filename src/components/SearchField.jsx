...
const SearchField = () => {
  ...
  return (
    <div className=' relative flex h-80 justify-center bg-hero-banner bg-cover bg-center items-center'>
      {/* Hero Banner */}
      <div className="absolute w-full h-full top-0 left-0 bg-black opacity-50 z-10">

      </div>

      {/* Search */}
      <div className="z-20 my-auto relative flex items-center mx-auto w-full sm:w-1/2 px-10">
        <input value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyPress= {handleKeyPress} type="text" className="border border-gray-500 drop-shadow-lg p-4 w-full placeholder:text-slate-300 rounded-lg bg-white/30 text-white" placeholder='Search for images...' />
